...
public class ContentService {
    ...
    public List<List<Content>> getContentsByChapterIds(final List<UUID> chapterIds) {
        final List<List<Content>> result = new ArrayList<>(chapterIds.size());
        ...
        final Map<UUID, List<Content>> contentsByChapterId = getContentSortedByChapterId(chapterIds);
        ...
        for (final UUID chapterId : chapterIds) {
            final List<Content> contents = contentsByChapterId.getOrDefault(chapterId, Collections.emptyList());
    ...
    public Map<UUID, List<Content>> getContentSortedByChapterId(final List<UUID> chapterIds) {
    ...
    public List<List<Content>> getContentWithNoSection(final List<UUID> chapterIds) {
        ...
        final Stream<Set<ContentEntity>> requiredContentStream = sectionEntities.stream()
                .map(SectionEntity::getStages)
                .flatMap(Collection::stream)
                .map(StageEntity::getRequiredContents);

        final Stream<Set<ContentEntity>> optionalContentStream = sectionEntities.stream()
                .map(SectionEntity::getStages)
                .flatMap(Collection::stream)
        ...
        final Map<UUID, List<Content>> contentsWithNoSectionByChapterId = contentEntities.stream()
        ...
        for (final UUID chapterId : chapterIds) {
            final List<Content> contents = contentsWithNoSectionByChapterId.getOrDefault(chapterId, Collections.emptyList());
            result.add(contents);
        }
